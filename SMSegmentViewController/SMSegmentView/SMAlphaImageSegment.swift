...
open class SMAlphaImageSegment: SMBasicSegment {
    ...
    func resetContentFrame() {
        ...
        let imageViewFrame = CGRect(x: margin, y: margin,
          width: self.frame.size.width - margin*2, height: self.frame.size.height - margin*2)
    ...
    func startAnimationToAlpha(_ alpha: CGFloat) {
        UIView.animate(withDuration: self.animationDuration,
          ...
          .beginFromCurrentState, animations: { () -> Void in
